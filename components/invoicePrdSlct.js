...
export default function Example({ isSelection, selectOrEdit, indx, ln }) {
    ...
    const itemCls = (active, disabled) =>
        ...
        ${active && !disabled ? 'bg-[#F4F3F9] text-[var(--endeavour)]' : 'text-[var(--port-gore)]'}`
        ...
        <div>
            ...
            {open && typeof window !== 'undefined' && createPortal(
                <div
                    ...
                    className="divide-y divide-[#F4F3F9] rounded-xl bg-white shadow-lg border border-[#F4F3F9] min-w-[160px]"
                >
                    <div className="px-1 py-1">
                        <button
                            className={itemCls(false, !isSelection)}
                            disabled={!isSelection}
                            onMouseEnter={e => !(!isSelection) && e.currentTarget.classList.add('bg-[#F4F3F9]', 'text-[var(--endeavour)]')}
                            onMouseLeave={e => e.currentTarget.classList.remove('bg-[#F4F3F9]', 'text-[var(--endeavour)]')}
