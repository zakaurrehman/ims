...
const AVATAR_COLORS = ['var(--brand)', 'var(--ok-text)', 'var(--warn-text)', '#7c3aed', 'var(--bad-text)', '#0e7490'];
const colorFor = (key = '') => {
    let h = 0;
        ...
        <div className='flex flex-col p-3' style={{ minHeight: 240 }}>
            ...
            <div className='flex-1 overflow-y-auto pr-1' style={{ maxHeight: '50vh' }}>
                {loading ? (
                    ...
                    <div className='flex flex-col items-center justify-center py-8 gap-1'>
                        <MessageSquare className='w-5 h-5' style={{ color: 'var(--line)' }} />
                            ...
                            return (
                                <li key={c.id} className='flex items-start gap-2'>
                                    ...
                                    <div className='min-w-0 flex-1'>
                                        ...
                                        <div className='rounded-lg px-2.5 py-1.5 mt-0.5 whitespace-pre-wrap break-words' style={{ fontSize: '0.7rem', color: 'var(--port-gore)', background: mine ? 'var(--bg-subtle)' : 'var(--bg-subtle)', border: '1px solid #eef5fc' }}>
                                            {c.text}
                                        </div>
                                    </div>
                                </li>
            ...
            <div className='flex items-end gap-2 mt-2 pt-2' style={{ borderTop: '1px solid #eef5fc' }}>
                <textarea
                    ...
                    style={{ fontSize: '0.72rem', borderColor: 'var(--line)', background: 'var(--bg-subtle)', color: 'var(--port-gore)', fontFamily: 'inherit' }}
                />
                <button
                    onClick={send}
