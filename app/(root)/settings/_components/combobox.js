    ...
    <div className="w-36">
      <Listbox value={selected} onChange={(e) => setSelection(e)}>
        <div className="relative">
          <Listbox.Button className="cursor-pointer w-full h-8 rounded-full border border-[#E8EBF0] bg-white
          ...
          <Transition
            ...
            leaveTo="opacity-0"
          >
            <Listbox.Options className="absolute mt-1 max-h-60 w-full overflow-auto rounded-xl bg-white py-1 responsiveText shadow-lg border border-[#F3F5F8] focus:outline-none z-50 text-[0.75rem]">
              {languages.map((language, personIdx) => (
                <Listbox.Option
                  key={personIdx}
                  className={({ active }) =>
                    `relative cursor-pointer select-none py-2 pl-9 pr-4 ${active ? 'bg-[#F3F5F8] text-[var(--endeavour)]' : 'text-[var(--port-gore)]'}`
                  }
                  value={language}
                >
                  {({ selected, active }) => (
